...
const AllUsers: React.FC = () => {
    ...
    try {
      ...
      if (response.success) {
        const pending = response.friends
          .filter((friend: UserFriendsDTO) => friend.status === FriendshipStatus.pending)
          .map((friend: UserFriendsDTO) => friend.friend_id);
  ...
  const block_user = async (id: string) => {
    try {
      ...
      if (response.success) {
        // Məsələn, siyahını təzələyə bilərsiniz:
        fetchAllUsers();
      }
    } catch (error) {
  ...
  const send_friend_request = async (id: string) => {
    try {
      ...
      if (response.success) {
        // İstifadəçiyə bildiriş vermək və ya siyahını yeniləmək olar
      }
    } catch (error) {
            ...
            {filterUsers(users).map((user) => {
              ...
              return (
                <ListItem key={user.id} divider>
                  {isSmallScreen ? (
                    // Mobil görünüş: bütün elementlər mərkəzdə, vertikal düzülüş
                    <Box
                      sx={{
                        width: "100%",
                        display: "flex",
                    ...
                    // Masaüstü görünüş: avatar solda, mərkəzdə mətn və düymələr sağda
                    <>
                      <ListItemAvatar>
                        <Avatar
                          src={user.account.profile_picture || no_profile_photo}
